...
const links = [
  ["/lavagestor", "Dashboard"],
  ["/lavagestor/clientes", "Clientes"],
  ["/lavagestor/veiculos", "Veiculos"],
  ["/lavagestor/funcionarios", "Funcionarios"],
  ["/lavagestor/servicos", "Servicos"],
  ["/lavagestor/lavagens", "Lavagens"],
  ["/lavagestor/comissoes", "Comissoes"],
  ["/lavagestor/vales", "Vales"]
];

export function LavaGestorShell({ children }: { children: React.ReactNode }) {
    ...
    <main>
      <header className="border-b border-white/10 bg-black/20">
        <nav className="page-shell flex min-h-16 flex-wrap items-center justify-between gap-4 py-3">
          <Link className="text-lg font-black" href="/lavagestor">
            LavaGestor
          </Link>
    ...
    <section className="page-shell grid gap-4 py-8">
      <div className="panel max-w-xl p-6">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-300">
          As telas do LavaGestor exigem login e assinatura ativa.
        </p>
        {error ? <p className="mt-3 text-sm text-red-200">{error}</p> : null}
        <a className="button-primary mt-5" href={`${coreUrl}/login?next=/lavagestor`}>
...
export function RowsPreview({
  ...
  return (
    <section className="page-shell grid gap-5 py-8">
      <div>
        <p className="eyebrow">LavaGestor</p>
