...
export default async function AppPage() {
  ...
  const announcements = (announcementData ?? []) as Announcement[];
  ...
  return (
    <main className="safe-bottom min-h-screen bg-[#f5f8fb]">
        ...
        <div className="page-shell flex min-h-20 items-center justify-between gap-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="line-clamp-1 text-sm text-slate-500">{profile?.escola?.nome || "Sua escola"}</p>
            </div>
          </div>
          <LogoutButton />
        </div>
      ...
      <div className="page-shell grid gap-7 py-7">
        <section>
          <p className="text-sm font-bold text-[#176b5b]">{roleLabel(profile?.papel)}</p>
        ...
        <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
          {(isGuardian ? guardianShortcuts : staffShortcuts).map(({ label, description, Icon }) => (
      ...
      <nav className="fixed inset-x-0 bottom-0 border-t border-slate-200 bg-white/95 px-3 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 backdrop-blur md:hidden">
        <div className="mx-auto grid max-w-md grid-cols-4">
          ...
          <NavItem icon={<Bell size={21} />} label="Mural" />
          <NavItem icon={<ClipboardList size={21} />} label="Atividades" />
          <NavItem icon={<UsersRound size={21} />} label={isGuardian ? "Filhos" : "Turmas"} />
        </div>
      </nav>
    </main>
  );
