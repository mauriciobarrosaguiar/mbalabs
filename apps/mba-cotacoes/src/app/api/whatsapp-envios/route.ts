...
} from "@/lib/whatsapp/mba-cotacoes";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error("Erro ao listar envios de WhatsApp", error);
    return NextResponse.json({ envios: [] });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.action === "resend" || (body.tipoEnvio && body.vendedorId)) {
      if (!body.tipoEnvio || !body.vendedorId) {
        return NextResponse.json({ error: "Tipo de envio e vendedor são obrigatórios para reenviar." }, { status: 400 });
      }

      if (body.tipoEnvio === "link_cotacao") {
        const result = await resendQuotationLink({
      ...
      const orders = await generatePurchaseOrders(body.quotationId);
      const whatsapp = await sendWinnerOrderLinksByQuotation({
        quotationId: body.quotationId,
        origin,
        orders,
        vendedorId: body.vendedorId,
    ...
    return NextResponse.json(
      { error: resolveErrorMessage(error, "Erro no envio automático de WhatsApp.") },
      { status: 500 },
    );
  }
}

async function ensureQuotationAccess(
  auth: Awaited<ReturnType<typeof getCurrentAuthContext>>,
  quotationId: string,
) {
  if (!auth.isAuthenticated || !auth.isActive) {
    return { ok: false as const, status: 401, error: "Sessão expirada." };
  }

  if (!hasSupabaseConfig() || !hasSupabaseAdminConfig()) {
    return { ok: false as const, status: 409, error: "Supabase não configurado." };
  }
  ...
  if (data.tenant_id !== auth.tenantAccess?.tenantId) {
    return { ok: false as const, status: 403, error: "Sem permissão para esta cotação." };
  }

  return { ok: true as const };
}

async function resendQuotationLink(input: {
  quotationId: string;
  vendedorId: string;
  origin: string;
}) {
  ...
  const { data: quotation, error: quotationError } = await supabase
    .from("quotations")
    .select("id, tenant_id, module_type, pharmacy_id, buyer_company_name")
...
async function getBuyerCompanyName(supabase: ReturnType<typeof createSupabaseAdminClient>, quotation: Record<string, any>) {
...
function summarizeSingle(result: Awaited<ReturnType<typeof sendWhatsAppMbaCotacoes>>) {
  return {
    total: 1,
    enviado: result.status === "enviado" ? 1 : 0,
    falhou: result.status === "falhou" ? 1 : 0,
    pendente: result.status === "pendente" ? 1 : 0,
    ignorado: result.skipped ? 1 : 0,
    results: [result],
  };
}
