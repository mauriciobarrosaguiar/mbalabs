...
import { canFinishQuotation, isQuotationClosed } from "@/lib/quotation-status";
  ...
  try {
    ...
    return NextResponse.json({
      ...
      links: result.sessions.map((session: { supplierId: string; publicToken: string }) => ({
        ...
        url: `${request.nextUrl.origin}/${prefix}/responder/${session.publicToken}`,
      })),
    });
  } catch (error) {
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (body.action === "duplicate") {
      ...
      const duplicatedItems = (itemRows ?? []).map((item: Record<string, any>) => {
