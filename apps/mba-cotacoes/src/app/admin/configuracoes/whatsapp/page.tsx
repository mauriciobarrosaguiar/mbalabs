...
import { getWhatsappGlobalConfigForAdmin } from "@/lib/whatsapp/mba-cotacoes";

export const dynamic = "force-dynamic";
export const revalidate = 0;

export default async function WhatsappMbaCotacoesAdminPage({
  searchParams,
}: {
  searchParams: Promise<Record<string, string | undefined>>;
}) {
  const [auth, config, params] = await Promise.all([
    requireSuperAdmin("/admin/configuracoes/whatsapp"),
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <h2 className="mt-3 text-xl font-semibold text-slate-950">
                Envio automático pelo WhatsApp oficial do MBA Cotações
              </h2>
              <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-700">
                Apenas o Admin Master vê esta configuração. Farmácias continuam usando somente os botões da cotação,
                sem QR Code, token, API ou WhatsApp próprio.
              </p>
            </div>
          ...
          <CardContent>
            <form action={saveWhatsappSettingsAction} className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              <div className="space-y-2">
                ...
                <select
                  name="provider"
                  defaultValue={config?.provider ?? "evolution_api"}
              ...
              <div className="space-y-2">
                ...
                <Input
                  name="api_token"
                  type="password"
                  placeholder={config?.api_token_configurado ? "token configurado — preencha só para trocar" : "cole o token/API key"}
                />
              </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-sm leading-6 text-muted-foreground">
                Valida se existe provider e dados mínimos configurados. Não envia mensagem para vendedores.
              </p>
              ...
              <form action={sendWhatsappTestMessageAction} className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    defaultValue="Mensagem de teste do MBA Cotações."
                  />
                </div>
                <Button type="submit" variant="outline">
