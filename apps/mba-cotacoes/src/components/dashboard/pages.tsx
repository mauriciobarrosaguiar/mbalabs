...
import { DemoCrudTable, type DemoCrudRow } from "@/components/forms/demo-crud-table";
import { ImportWizard } from "@/components/imports/import-wizard";
import { DemoQuotationTable } from "@/components/quotations/demo-quotation-table";
import { GeneratePurchaseOrdersButton } from "@/components/quotations/generate-purchase-orders-button";
...
import { QuotationBasicEditor } from "@/components/quotations/quotation-basic-editor";
import { BackButton, QuotationPageActions } from "@/components/quotations/quotation-page-actions";
import { SupplierLinksTable } from "@/components/quotations/supplier-links-table";
import { WinnerOrderLinks } from "@/components/quotations/winner-order-links";
import { CreateMissingQuotationButton, WinnerPendingActions } from "@/components/quotations/winner-pending-actions";
...
import { getUnitLabel, productTypeLabels } from "@/lib/constants";
import {
  formatCurrency,
  formatDate,
  formatInteger,
...
import { getRuntimeSummary } from "@/lib/runtime-mode";
import {
...
import { savePaymentSettingsAction } from "@/lib/actions/payment-settings";
import { createUserAccessAction, updateUserAccessAction } from "@/lib/actions/users";
import { listManagedUsers, listTenantOptions, type ManagedUser, type TenantOption } from "@/lib/auth/users";
                  ...
                  <TableRow key={quotation.id}>
                    ...
                    <TableCell>
                      <Button asChild variant="outline" size="sm">
                        <Link href={`/app/licitacoes/${quotation.id}`}>Ver</Link>
                      </Button>
                    </TableCell>
                  </TableRow>
                );
      ...
      <PageStack>
        ...
        <div className="flex flex-wrap gap-3">
          {tenantType !== "distributor_bidding" ? (
            <Button asChild>
              <Link href="/app/cotacoes-farmacia/nova">Criar cotação farmácia</Link>
            </Button>
          ) : null}
          {tenantType !== "pharmacy" ? (
            <Button asChild variant="outline">
              <Link href="/app/licitacoes/nova">Criar cotação licitação</Link>
            </Button>
          ) : null}
        </div>
      </PageStack>
  ...
  return (
    <Card className="border-amber-200 bg-amber-50/70">
      <CardContent className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Button asChild variant="outline">
            <Link href="/app/configuracoes/supabase">Configurar Supabase</Link>
          </Button>
          {tenantType !== "distributor_bidding" ? (
            <Button asChild>
              <Link href="/app/cotacoes-farmacia/nova">Nova cotação farmácia</Link>
            </Button>
          ) : null}
          {tenantType !== "pharmacy" ? (
            <Button asChild variant="outline">
              <Link href="/app/licitacoes/nova">Nova cotação licitação</Link>
            </Button>
          ) : null}
        </div>
      </CardContent>
    ...
    <PageStack>
      <Card className="border-red-200 bg-red-50/80">
        <CardContent className="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between">
          ...
          <Button asChild>
            <Link href="/app/configuracoes/supabase">Ver configuração</Link>
          </Button>
        </CardContent>
      </Card>
          ...
          <TableBody>
            {quotations.length === 0 ? (
              ...
              quotations.map((quotation) => {
                ...
                const base = quotation.moduleType === "bidding" ? "/app/licitacoes" : "/app/cotacoes-farmacia";
                return (
                  <TableRow key={quotation.id}>
                    <TableCell>{[quotation.processNumber, quotation.bidNumber].filter(Boolean).join(" / ") || "-"}</TableCell>
                    <TableCell className="font-medium">{quotation.name}</TableCell>
...
async function GeneratedOrdersPage({
  ...
  const exportHref = `/api/export/pedidos-gerados${exportSearchParams.size > 0 ? `?${exportSearchParams.toString()}` : ""}`;
  const pageHref =
    forcedModule === "pharmacy" ? "/app/pedidos-gerados-farmacia" :
    forcedModule === "bidding" ? "/app/pedidos-gerados-licitacao" :
    "/app/pedidos-gerados";
  const moduleTitle =
            ...
            <Table>
              ...
              <TableBody>
                {rows.map(({ quotation, order }) => {
                  ...
                  const detailHref = `/app/${order.moduleType === "bidding" ? "licitacoes" : "cotacoes-farmacia"}/${quotation.id}/pedidos`;
                  return (
                    <TableRow key={order.id}>
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-2">
                          ...
                          <Button asChild variant="outline" size="sm">
                            <Link href={`/api/export/pedido/${order.publicToken}`}>Exportar</Link>
                          </Button>
                        </div>
                      </TableCell>
                    </TableRow>
      ...
      <TwoColumn>
        <Card>
          ...
          <CardContent className="space-y-3 text-sm text-muted-foreground">
            ...
            <Button asChild variant="outline">
              <Link href="/app/usuarios">Ver usuários</Link>
            </Button>
          </CardContent>
        </Card>
        <Card>
...
async function QuotationList({ moduleType }: { moduleType: "pharmacy" | "bidding" }) {
  ...
  const base = moduleType === "pharmacy" ? "/app/cotacoes-farmacia" : "/app/licitacoes";
...
async function NewQuotationPage({ moduleType }: { moduleType: "pharmacy" | "bidding" }) {
  ...
  return (
    <PageStack>
      ...
      />
      <BackButton fallbackHref={moduleType === "pharmacy" ? "/app/cotacoes-farmacia" : "/app/licitacoes"} />
      <NewQuotationForm
  ...
  return (
    <PageStack>
      <HeaderBlock
        ...
        actionHref={canGenerateQuotationOrders(quotation.status) ? `/app/cotacoes-farmacia/${quotationId}/pedidos` : undefined}
      />
      ...
      <Card>
        ...
        <CardContent className="space-y-4 p-0">
          ...
          <div className="flex flex-wrap gap-2 p-4">
            <Button asChild>
              <Link href={`/app/licitacoes/${quotationId}/pedidos`}><ReceiptText className="h-4 w-4" />Ver pedidos por fornecedor</Link>
            </Button>
            <Button asChild variant="outline">
              <Link href={`/api/export/licitacao/${quotationId}`}><Download className="h-4 w-4" />Exportar Excel</Link>
            </Button>
          </div>
        </CardContent>
      </Card>
...
async function ComparativeMap({ quotationId }: { quotationId: string }) {
  ...
  return (
    <PageStack>
      ...
      />
      <BackButton fallbackHref={`/app/licitacoes/${quotationId}`} />
      <Card>
        <CardContent className="overflow-x-auto p-0">
          <Table>
            <TableHeader>
  ...
  return (
    <PageStack>
      ...
      <div className="flex flex-wrap justify-end gap-2">
        {orders.length > 0 ? (
          <Button asChild variant="outline">
            <Link href={`/api/export/pedidos/${quotationId}`}>
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <Button asChild variant="outline" className="w-full">
          <Link href="/app/cotacoes-farmacia">Abrir cotações</Link>
        </Button>
      </CardContent>
    </Card>
  );
