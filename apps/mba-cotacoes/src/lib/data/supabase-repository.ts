...
import { parseLegacyPurchaseOrderReview } from "@/lib/purchase-order-review-legacy";
import { isQuotationDeleted, isQuotationInProgress } from "@/lib/quotation-status";
import { buildBiddingAnalysis } from "@/lib/services/bidding-analysis";
...
async function enrichPurchaseOrdersWithSuppliers(supabase: DbClient, orders: PurchaseOrder[]) {
  ...
  const suppliersById = new Map<string, Pick<Supplier, "nome" | "empresa" | "whatsapp">>(
    ((data ?? []) as Array<Pick<Supplier, "id" | "nome" | "empresa" | "whatsapp">>)
