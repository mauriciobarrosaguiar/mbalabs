...
} from "@/lib/services/bidding-analysis";
import {
  calculateSellerRow,
...
} from "@/lib/purchase-order-review-legacy";
import {
...
export async function saveSupabasePublicResponse(token: string, input: {
  ...
  const itemPayload: Record<string, any>[] = publicSession.items.flatMap((item) => {
...
async function enrichPurchaseOrdersWithSuppliers(supabase: SupabaseClient, orders: PurchaseOrder[]) {
  ...
  const suppliersById = new Map<string, { nome?: string | null; empresa?: string | null; whatsapp?: string | null }>(
    ((data ?? []) as Array<{ id: string; nome?: string | null; empresa?: string | null; whatsapp?: string | null }>)
...
async function resolveQuotationProducts(
  ...
  if (error) throw error;
  return new Map(
    ((data ?? []) as Array<Record<string, any>>).map((product) => [product.id as string, product]),
  );
}
