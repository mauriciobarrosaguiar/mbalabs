...
export async function listLavaComissoesResumo() {
  ...
  const [funcionariosResult, comissoesResult, valesResult] = await Promise.all([
    client.from("lava_funcionarios").select("id,nome,telefone,ativo").eq("empresa_id", current.empresaId).order("nome", { ascending: true }).limit(300),
    client.from("lava_comissoes").select("id,funcionario_id,lavagem_id,valor,status,pago_em,created_at").eq("empresa_id", current.empresaId).in("status", ["pendente", "pago"]).order("created_at", { ascending: false }).limit(500),
    client.from("lava_vales").select("id,funcionario_id,valor,valor_descontado,descricao,data_vale,status,created_at").eq("empresa_id", current.empresaId).in("status", ["aberto", "parcial", "descontado"]).order("data_vale", { ascending: false }).limit(500)
  ]);
  ...
  const rows = funcionarios
    .map((funcionario) => {
      ...
      const valesComSaldo = valesFuncionario.filter((row) => ["aberto", "parcial"].includes(String(row.status)) && saldoVale(row) > 0);
      const totalPendente = sumRows(pendentes, "valor");
      const totalPago = sumRows(pagas, "valor");
      const totalValesAbertos = valesComSaldo.reduce((total, row) => total + saldoVale(row), 0);
      ...
      return {
        ...
        qtd_vales_abertos: valesComSaldo.length,
        comissoes_pendentes: pendentes,
        vales_abertos: valesComSaldo
      };
    })
...
function saldoVale(row: AnyRow) {
  return Math.max(Number(row.valor ?? 0) - Number(row.valor_descontado ?? 0), 0);
}
