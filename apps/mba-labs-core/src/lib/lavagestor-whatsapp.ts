...
import {
  ...
  resolveLavaEmpresaIdFromWhatsappEnvio
...
const OPERATIONAL_AUTO_EVENTS = new Set([
...
export function buildAgendamentoConfirmacaoMessage({ cliente, empresa, servico, quando }: { cliente?: unknown; empresa?: unknown; servico?: unknown; quando?: unknown }) {
...
export async function testWhatsappIntegration(current: Current) {
  ...
  try {
    ...
    if (provider === "manual") result = { ok: true, detail: "Modo manual via wa.me pronto." };
    else if (provider === "evolution") result = await testEvolution(row);
    else result = await testCloudApi(row);
...
export async function enqueueWhatsappMessage(first: DbClient | Current, second: QueueInput | (Record<string, unknown> & { evento?: string; telefone?: string; mensagem?: string })) {
  if (isSupabaseClient(first)) return enqueueWhatsappMessageLegacy(first, second as QueueInput);
...
export async function approveWhatsappMessage(current: Current, envioId: string) {
  const client = (await getSupabaseServer()) as DbClient;
  const empresaId = await resolveLavaEmpresaIdFromWhatsappEnvio(client, envioId);
...
export async function sendWhatsappMessage(current: Current, envioId: string) {
  ...
  const empresaId = String(envio.empresa_id ?? "");
  await assertLavaEmpresaAccess(current, empresaId);
...
export async function sendPendingWhatsappMessages(current: Current) {
  if (!current.empresaId) throw new Error("Empresa nao identificada.");
  const client = (await getSupabaseServer()) as DbClient;
  const { data, error } = await client.from("lava_whatsapp_envios").select("id,status").eq("empresa_id", current.empresaId).in("status", ["pendente", "aprovado", "erro"]).order("created_at", { ascending: true }).limit(25);
  if (error) throw new Error(error.message);
  let synced = 0;
  let failed = 0;
  let skipped = 0;
...
export async function sendViaWhatsAppCloudApi(params: { integration: Row; phone: string; message: string }) {
  ...
  const response = await fetch(`${baseUrl}/${encodeURIComponent(phoneNumberId)}/messages`, {
    ...
    body: JSON.stringify({ messaging_product: "whatsapp", to: params.phone, type: "text", text: { preview_url: false, body: params.message } }),
...
export async function canSendAutomaticMessage(current: Current, evento: string, clienteId?: string | null) {
  ...
  if (clienteId) {
    ...
    const { count } = await client.from("lava_whatsapp_envios").select("id", { count: "exact", head: true }).eq("empresa_id", current.empresaId).eq("cliente_id", clienteId).gte("created_at", since.toISOString()).neq("status", "cancelado");
    if ((count ?? 0) >= integration.limiteMensagensClienteDia) return { ok: false, error: `Limite diario de ${integration.limiteMensagensClienteDia} mensagens para este cliente.` };
  }
  return { ok: true };
}

export async function avoidDuplicateWhatsappMessage(params: { empresaId: string; evento?: string | null; lavagemId?: string | null; agendamentoId?: string | null; automacaoId?: string | null; clienteId?: string | null }) {
  const client = (await getSupabaseServer()) as DbClient;
  let query = client
    .from("lava_whatsapp_envios")
    .select("id,status")
    ...
    .in("status", ["enviado", "pendente", "enviando", "aprovado", "aguardando_aprovacao"])
    .limit(1);
...
export async function enqueueAutomationQueue(client: DbClient, input: QueueInput) {
  ...
  const { data, error } = await client
    .from("lava_automacao_fila")
    .insert({ empresa_id: input.empresaId, automacao_id: input.automacaoId || null, cliente_id: input.clienteId || null, lavagem_id: input.lavagemId || null, agendamento_id: input.agendamentoId || null, canal: "whatsapp", tipo: input.tipo || "manual", mensagem: input.mensagem, status: "pronto", agendado_para: input.agendadoPara || new Date().toISOString() })
    .select("id,status")
    .single();
...
export async function markWhatsappAsSent(client: DbClient, empresaId: string, id: string, source: "fila" | "envio" = "fila") {
  ...
  const { error } = await client.from(table).update({ status: "enviado_manual", enviado_em: new Date().toISOString(), erro: null }).eq("id", id).eq("empresa_id", empresaId);
  return error ? { ok: false, error: String(error.message ?? "Nao foi possivel marcar como enviado.") } : { ok: true };
}

export async function markWhatsappEnvioAsSent(current: Current, envioId: string) {
  const client = (await getSupabaseServer()) as DbClient;
  const empresaId = await resolveLavaEmpresaIdFromWhatsappEnvio(client, envioId);
  await assertLavaEmpresaAccess(current, empresaId);
  return markWhatsappAsSent(client, empresaId, envioId, "envio");
}
...
export async function sendWhatsappMessageIfConfigured(client: DbClient, input: QueueInput) {
  ...
  return { ...queued, provider: "manual", url: buildWhatsappUrl(input.telefone, input.mensagem) };
}
...
async function enqueueWhatsappMessageLegacy(client: DbClient, input: QueueInput) {
  ...
  const payload = { empresa_id: input.empresaId, cliente_id: input.clienteId || null, lavagem_id: input.lavagemId || null, agendamento_id: input.agendamentoId || null, automacao_id: input.automacaoId || null, automacao_fila_id: input.automacaoFilaId || null, evento, telefone: normalizePhoneBR(input.telefone) || input.telefone || null, mensagem: input.mensagem, mensagem_gerada_por: "modelo", provider: "manual", status: "pronto", precisa_aprovacao: false, agendado_para: input.agendadoPara || null };
...
async function enqueueWhatsappMessageForCurrent(current: Current, params: Row) {
  ...
  let empresaId = explicitEmpresaId ?? current.empresaId;

  if (lavagemId) {
    try {
      empresaId = await resolveLavaEmpresaIdFromLavagem(client, lavagemId);
      await assertLavaEmpresaAccess(current, empresaId);
    } catch (err) {
      return { ok: false, error: err instanceof Error ? err.message : "Sem acesso a esta lavagem." };
    }
  }
  ...
  try {
    ...
    return { ok: false, error: err instanceof Error ? err.message : "Sem acesso a esta empresa." };
  }
  ...
  const insert = await client.from("lava_whatsapp_envios").insert({
    ...
    aprovado_por: isAutomaticTotal ? current.usuario?.id ?? null : null,
    aprovado_em: isAutomaticTotal ? new Date().toISOString() : null,
    erro,
...
async function findExistingLegacyMessage(client: DbClient, input: QueueInput, evento: string) {
  let query = client.from("lava_whatsapp_envios").select("id,status").eq("empresa_id", input.empresaId).eq("evento", evento).neq("status", "cancelado").limit(1);
...
async function testEvolution(row: Row) {
  ...
  const response = await fetchWithTimeoutRetry(`${apiUrl}/instance/connectionState/${encodeURIComponent(instance)}`, { headers: { apikey: apiKey }, timeoutMs: 20000, retryTimeoutMs: 25000, retryOnTimeout: true });
  if (!response.ok) throw new Error(await responseErrorMessage(response, "Evolution API"));
...
async function testCloudApi(row: Row) {
  ...
  const response = await fetch(`${baseUrl}/${encodeURIComponent(phoneNumberId)}?fields=id,display_phone_number,verified_name`, { headers: { Authorization: `Bearer ${token}` }, signal: AbortSignal.timeout(15000) });
  if (!response.ok) throw new Error(await responseErrorMessage(response, "WhatsApp Cloud API"));
...
async function responseErrorMessage(response: Response, providerLabel: string) {
  ...
  if (providerLabel === "WhatsApp Cloud API" && /template|24.?hour|outside/i.test(message)) return `${message}. Mensagem exige template aprovado no WhatsApp Cloud API.`;
  return redactSensitiveText(message);
}

async function fetchWithTimeoutRetry(url: string, options: RequestInit & { timeoutMs?: number; retryTimeoutMs?: number; retryOnTimeout?: boolean }) {
...
function friendlyFetchError(err: unknown) {
  if (isAbortError(err)) return new Error("A Evolution demorou para responder e a operação expirou. Se estiver usando Render gratuito, aguarde a instância acordar e tente enviar novamente.");
...
async function setWhatsappError(client: DbClient, empresaId: string | null, envioId: string, error: string, tentativas: number) {
  if (!empresaId) return;
  await client.from("lava_whatsapp_envios").update({ status: "erro", erro: redactSensitiveText(error), tentativas }).eq("id", envioId).eq("empresa_id", empresaId);
}
...
function emptyIntegration(): WhatsappIntegrationView {
  return normalizeIntegration({ provider: "manual", status: "inativo", modo_envio: "manual", horario_envio_inicio: "08:00", horario_envio_fim: "18:00", limite_mensagens_cliente_dia: 5, limite_tentativas: 3 });
}
...
function defaultTemplate(evento: string) {
  const templates: Record<string, string> = {
    ...
    veiculo_pronto: "Ola, {cliente}! Seu veiculo {veiculo} esta pronto para retirada na {empresa}. Valor total: {total}. Status do pagamento: {status_pagamento}. Valor pendente: {valor_pendente}.",
    pagamento_recebido: "Ola, {cliente}! Pagamento confirmado na {empresa}. Valor recebido: {valor}. Total da lavagem: {total}. Obrigado pela preferencia! Seu recibo ja esta liberado.",
