      ...
      <div className="mx-auto grid w-full max-w-3xl gap-4">
        <header className="rounded-[26px] border border-emerald-200 bg-white p-4 shadow-xl shadow-emerald-950/10 sm:p-5">
          <div className="flex items-start gap-3">
            <Link href="/apps/dronegestor" className="mt-0.5 grid size-10 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white text-slate-700 sm:size-11" aria-label="Voltar"><ArrowLeft size={20}/></Link>
            <div className="min-w-0 flex-1"><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[.12em] text-emerald-700 sm:text-xs"><BookOpenCheck size={16}/> DroneGestor Agro</div><h1 className="mt-1 text-2xl font-black leading-tight text-slate-950 sm:text-3xl">Biblioteca de produtos e bulas</h1><p className="mt-2 text-sm leading-5 text-slate-600">Catálogo oficial para identificação do produto + revisão técnica da instrução de preparo.</p></div>
          </div>
        </header>

        <section className="rounded-[26px] border border-slate-200 bg-white p-4 sm:p-5">
          <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
            <div className="flex min-h-12 min-w-0 items-center rounded-xl border border-slate-200 focus-within:border-emerald-500"><Search className="ml-3 shrink-0 text-slate-400" size={18}/><input className="min-w-0 flex-1 px-3 text-[16px] font-semibold text-slate-950 outline-none" value={query} placeholder="Nome, registro ou ingrediente ativo" onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void searchProducts(); }}/></div>
            <button type="button" disabled={loading} onClick={() => void searchProducts()} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 text-sm font-black text-white disabled:opacity-60 sm:w-auto sm:min-w-24">{loading ? <Loader2 className="animate-spin" size={18}/> : <Search size={18}/>} Buscar</button>
          </div>
        ...
        {selected && edit && <section className="rounded-[26px] border border-emerald-200 bg-white p-4 sm:p-5">
          <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0 flex-1"><h2 className="text-xl font-black text-slate-950">{selected.name}</h2><p className="mt-1 text-sm text-slate-500">{selected.registration ? `Registro ${selected.registration}` : "Registro não identificado"}{selected.formulation ? ` • ${selected.formulation}` : ""}</p>{selected.activeIngredient && <p className="mt-1 text-sm text-slate-600">{selected.activeIngredient}</p>}</div>{(() => { const info = badge(edit.status); return <span className={`inline-flex max-w-full items-center gap-1 rounded-full px-3 py-1 text-xs font-black ${info.cls}`}><info.Icon size={14}/>{info.text}</span>; })()}</div>
        ...
        <section className="grid gap-3">
          {items.map((item) => { const info = badge(item.verification.status); return <button type="button" key={item.key || `${item.name}-${item.registration}`} onClick={() => choose(item)} className="rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0 flex-1"><strong className="block text-base text-slate-950">{item.name}</strong><span className="mt-1 block text-xs text-slate-500">{item.registration ? `Registro ${item.registration}` : "Registro não identificado"}{item.formulation ? ` • ${item.formulation}` : ""}</span>{item.activeIngredient && <span className="mt-1 block text-xs text-slate-600">{item.activeIngredient}</span>}</div><span className={`inline-flex max-w-full shrink-0 items-center gap-1 rounded-full px-2 py-1 text-[10px] font-black ${info.cls}`}><info.Icon size={12}/>{info.text}</span></div></button>; })}
        </section>
