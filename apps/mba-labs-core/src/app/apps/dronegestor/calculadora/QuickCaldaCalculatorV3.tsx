      ...
      <div className="mx-auto grid w-full max-w-xl gap-4">
        <header className="rounded-[26px] border border-emerald-200 bg-white p-4 shadow-xl shadow-emerald-950/10 sm:p-5">
          <div className="flex items-start gap-3">
            <Link href="/selecionar-app" className="mt-0.5 grid size-11 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white text-slate-700" aria-label="Voltar para a MBA Labs">
              <ArrowLeft size={20}/>
            </Link>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[.12em] text-emerald-700 sm:text-xs"><Calculator size={15}/> Calculadora de Calda</div>
              <h1 className="mt-1 text-[27px] font-black leading-[1.02] tracking-[-0.035em] text-slate-950 sm:text-[34px]">Calda fácil</h1>
              <p className="mt-2 text-[14px] leading-5 text-slate-600">Digite os dados da aplicação e o nome dos produtos para calcular cada carga.</p>
            </div>
          </div>
        </header>
        ...
        <section className="rounded-[26px] border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <strong className="text-slate-950">4. Produtos e doses</strong>
              <p className="mt-1 text-sm leading-5 text-slate-500">Digite apenas o nome do produto e informe a dose. Não há pesquisa ou lista automática.</p>
            </div>
          </div>
            ...
            {products.map((product, index) => (
              <div key={product.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                <div className="mb-3 flex items-center justify-between gap-3">
                  ...
                  {products.length > 1 && (
                    <button type="button" onClick={() => removeProduct(product.id)} className="inline-flex items-center gap-1 text-xs font-black text-red-600">
                ...
                <div className="grid gap-3">
                  <label className="grid gap-1.5 text-sm font-bold text-slate-700">
                    ...
                    <input
                      type="text"
                      className="min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-[16px] font-semibold text-slate-950 outline-none placeholder:text-slate-400 focus:border-emerald-500"
                      value={product.name}
                  ...
                  <div className="grid grid-cols-[minmax(0,1fr)_128px] gap-2">
                    ...
                    <label className="grid gap-1.5 text-sm font-bold text-slate-700">
                      Unidade
                      <select
                        className="min-h-12 w-full rounded-xl border border-slate-200 bg-white px-2 text-[15px] font-semibold text-slate-900 outline-none focus:border-emerald-500"
                        value={product.unit}
          ...
          <button type="button" onClick={addProduct} className="mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-dashed border-emerald-300 bg-emerald-50 px-3 text-sm font-black text-emerald-800">
            <Plus size={17}/> Adicionar outro produto
          </button>
        </section>

        <section className="rounded-[26px] border border-emerald-200 bg-[linear-gradient(145deg,#ecfdf5,#f0fdfa)] p-4 shadow-sm sm:p-5">
          <div className="flex items-center gap-2 text-emerald-950"><Droplets size={20}/><strong>Resultado</strong></div>

          {!result.ready ? (
            <div className="mt-4 rounded-2xl bg-white/85 px-4 py-7 text-center text-sm font-bold leading-6 text-slate-500">Informe hectares, litros do misturador e vazão em L/ha.</div>
          ) : (
            <>
              <div className="mt-4 grid grid-cols-2 gap-2">
                <Metric label="Calda total" value={`${format(result.totalSprayL, 1)} L`}/>
                <Metric label="Total de cargas" value={String(result.mixes)}/>
                <Metric label="Área por carga cheia" value={`${format(result.areaPerMixer, 2)} ha`}/>
                <Metric label="Cargas cheias" value={String(result.fullMixes)}/>
              </div>

              {result.fullMixes > 0 && (
                <LoadCard
                  title="Carga cheia"
                  subtitle={`${format(result.mixerL, 1)} L • ${format(result.areaPerMixer, 2)} ha`}
              ...
              {result.hasPartial && (
                <LoadCard
                  title="Última carga"
                  subtitle={`${format(result.lastMixerL, 1)} L • ${format(result.lastAreaHa, 2)} ha`}
                  products={result.productResults}
                  mode="last"
                  tone="amber"
                />
                  ...
                  <div className="mt-2 grid gap-2">
                    {result.activeProducts.map((product) => (
                      <div key={product.id} className="flex items-center justify-between gap-3 text-sm">
                        <span className="min-w-0 truncate text-slate-600">{product.label}</span>
                        <strong className="shrink-0 text-slate-950">{format(product.total.value, 3)} {product.total.unit}</strong>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="rounded-2xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm leading-5 text-amber-950">
          <div className="flex items-start gap-2">
            ...
            <div><strong>Atenção:</strong> use a dose definida na receita/bula e confirme a compatibilidade antes da mistura. A calculadora apenas calcula as quantidades informadas.</div>
          </div>
        </div>

        <button onClick={clear} className="mb-4 flex min-h-12 items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 font-black text-slate-700">
          <RotateCcw size={18}/> Limpar calculadora
        </button>
      </div>
    </main>
  );
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-emerald-100 bg-white px-3 py-3">
      <span className="block text-[11px] font-black uppercase tracking-[.08em] text-slate-400">{label}</span>
...
function LoadCard({ title, subtitle, products, mode, tone }: { title: string; subtitle: string; products: CalculatedProduct[]; mode: "full" | "last"; tone: "green" | "amber" }) {
  ...
  return (
    <div className={`mt-3 rounded-xl p-3 ${toneClass}`}>
      <strong>{title}</strong>
      ...
      {validProducts.length ? (
        <div className="mt-3 grid gap-2">
          {validProducts.map((product) => {
            ...
            return (
              <div key={product.id} className="flex items-center justify-between gap-3 rounded-lg bg-white/85 px-3 py-2">
...
function NumberInput({ label, value, onChange, suffix, placeholder }: { label: string; value: string; onChange: (value: string) => void; suffix: string; placeholder?: string }) {
  return (
    <label className="grid gap-1.5 text-sm font-bold text-slate-700">
      {label}
      <div className="flex min-h-12 overflow-hidden rounded-xl border border-slate-200 bg-white focus-within:border-emerald-500">
        <input
          inputMode="decimal"
          type="text"
          className="min-w-0 flex-1 px-3 text-[16px] font-black text-slate-950 outline-none placeholder:font-bold placeholder:text-slate-400"
          value={value}
          placeholder={placeholder}
          onChange={(event) => onChange(event.target.value.replace(/[^0-9,.]/g, ""))}
        />
        {suffix && <span className="grid shrink-0 place-items-center border-l border-slate-200 bg-slate-50 px-3 text-xs font-black text-slate-500">{suffix}</span>}
      </div>
    </label>
  );
}
