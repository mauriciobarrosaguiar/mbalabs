...
import { ArrowLeft, Calculator, Droplets, FlaskConical, ListOrdered, Plus, RotateCcw, ShieldAlert, Trash2 } from "lucide-react";
...
type Product = { id: string; name: string; dose: string; unit: DoseUnit; formulation: Formulation };
...
function newProduct(index: number): Product {
  return { id: `${Date.now()}-${index}-${Math.random().toString(36).slice(2, 7)}`, name: "", dose: "", unit: "mL/ha", formulation: "" };
}

export function QuickCaldaCalculatorV2() {
  ...
  const result = useMemo(() => {
    ...
    const productResults: CalculatedProduct[] = products.map((product, index) => {
      ...
      const info = formulationInfo(product.formulation);
      return {
        ...product,
    ...
    const sequenceReady = activeProducts.length > 0 && activeProducts.every((product) => formulationInfo(product.formulation).automatic);
    const orderedProducts = [...activeProducts].sort((a, b) => a.order - b.order || a.label.localeCompare(b.label, "pt-BR"));
    const pendingSequence = activeProducts.filter((product) => !formulationInfo(product.formulation).automatic);
      ...
      <div className="mx-auto grid w-full max-w-xl gap-4">
        <header className="rounded-[26px] border border-emerald-200 bg-white p-4 shadow-xl shadow-emerald-950/10 sm:p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-2 text-[14px] leading-5 text-slate-600">Calcule a carga e receba a sequência e a receita de preparo.</p>
            </div>
          </div>
        </header>
        ...
        <section className="rounded-[26px] border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1"><strong className="text-slate-950">4. Produtos, doses e formulação</strong><p className="mt-1 text-sm leading-5 text-slate-500">A formulação é usada para sugerir a ordem de entrada no misturador.</p></div>
          </div>
            ...
            {products.map((product, index) => (
              <div key={product.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                ...
                <div className="grid gap-3">
                  ...
                  <label className="grid gap-1.5 text-sm font-bold text-slate-700">Formulação / tipo
                    <select className="min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-[15px] font-semibold text-slate-900 outline-none focus:border-emerald-500" value={product.formulation} onChange={(e) => updateProduct(product.id, { formulation: e.target.value as Formulation })}>
                      {FORMULATIONS.map((item) => <option key={item.value || "empty"} value={item.value}>{item.label}</option>)}
                    </select>
                  </label>
                </div>
              </div>
            ...
            <>
              ...
              <div className="mt-4 rounded-2xl border border-emerald-200 bg-white p-4">
                ...
                <div className="mt-3 grid gap-3 text-sm">
                  {result.fullMixes > 0 && <LoadCard title={`Carga cheia — ${format(result.mixerL, 1)} L`} subtitle={`Cobre aproximadamente ${format(result.areaPerMixer, 2)} ha.`} products={result.productResults} mode="full" tone="green"/>}
                  {result.hasPartial && <LoadCard title={`${result.fullMixes > 0 ? "Última carga" : "Carga única"} — ${format(result.lastMixerL, 1)} L`} subtitle={`Cobre ${format(result.lastAreaHa, 2)} ha.`} products={result.productResults} mode="last" tone="amber"/>}
                </div>
              </div>
              ...
              {result.activeProducts.length > 0 && <>
                <section className="mt-4 rounded-2xl border border-sky-200 bg-sky-50 p-4">
                  ...
                  <p className="mt-1 text-xs leading-5 text-sky-900">Referência operacional geral por formulação. Se rótulo, bula, receita ou RT indicar outra sequência, a orientação específica prevalece.</p>
                  <div className="mt-3 grid gap-2">
                    {result.orderedProducts.filter((product) => formulationInfo(product.formulation).automatic).map((product, index) => (
                      <div key={product.id} className="flex items-center gap-3 rounded-xl bg-white px-3 py-2.5">
                        <span className="grid size-7 shrink-0 place-items-center rounded-full bg-sky-700 text-xs font-black text-white">{index + 1}</span>
                        <div className="min-w-0"><strong className="block truncate text-sm text-slate-900">{product.label}</strong><span className="text-xs text-slate-500">{product.formulationLabel}</span></div>
                      </div>
                    ))}
                  </div>
                  {result.pendingSequence.length > 0 && <div className="mt-3 rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm leading-5 text-amber-950">
                    <strong>Conferência obrigatória:</strong> {result.pendingSequence.map((product) => product.label).join(", ")} não recebeu posição automática. Confira a sequência específica antes de misturar.
                  </div>}
                </section>
                ...
                <section className="mt-4 rounded-2xl border border-violet-200 bg-violet-50 p-4">
                  ...
                  <p className="mt-1 text-xs leading-5 text-violet-900">Use esta receita somente depois de conferir compatibilidade, bula/receita e a formulação de cada produto.</p>
              ...
              {result.activeProducts.length > 0 && <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
                <strong className="text-sm text-slate-900">Total de produto para toda a área</strong>
                <div className="mt-2 grid gap-2">
                  {result.activeProducts.map((product) => <div key={product.id} className="flex items-center justify-between gap-3 text-sm"><span className="min-w-0 truncate text-slate-600">{product.label}</span><strong className="shrink-0 text-slate-950">{format(product.total.value, 3)} {product.total.unit}</strong></div>)}
                </div>
              </div>}
            </>
          )}
        ...
        <div className="rounded-2xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm leading-5 text-amber-950">
          <div className="flex items-start gap-2"><ShieldAlert className="mt-0.5 shrink-0" size={18}/><div><strong>Antes de misturar:</strong> confirme dose, compatibilidade, formulação, ordem específica da bula/receita e qualidade da água. Não misture concentrados diretamente uns sobre os outros. Em caso de dúvida, interrompa o preparo e confirme com o RT.</div></div>
        </div>
...
function RecipeCard({ title, finalLiters, areaHa, products, mode, sequenceReady }: { title: string; finalLiters: number; areaHa: number; products: CalculatedProduct[]; mode: "full" | "last"; sequenceReady: boolean }) {
  const automaticProducts = products.filter((product) => product.valid && formulationInfo(product.formulation).automatic);
  const pending = products.filter((product) => product.valid && !formulationInfo(product.formulation).automatic);
  return <div className="mt-3 rounded-2xl border border-violet-200 bg-white p-4">
    ...
    <ol className="mt-3 grid gap-2 text-sm leading-5 text-slate-700">
      <li><strong>1.</strong> Separe os produtos desta carga, confira nome, dose, formulação, validade e EPI.</li>
      <li><strong>2.</strong> Adicione água suficiente para iniciar e manter boa agitação/recirculação, sem completar o volume final.</li>
      {automaticProducts.map((product, index) => { const amount = mode === "full" ? product.full : product.last; return <li key={product.id}><strong>{index + 3}.</strong> Com a agitação ligada, adicione <strong>{format(amount.value, 3)} {amount.unit} de {product.label}</strong> ({product.formulationLabel}). Aguarde a dispersão/homogeneização prevista na bula antes do próximo produto.</li>; })}
      {pending.length > 0 && <li className="rounded-lg bg-amber-50 p-2 text-amber-950"><strong>{automaticProducts.length + 3}.</strong> Antes de adicionar <strong>{pending.map((product) => product.label).join(", ")}</strong>, confirme a posição correta na bula/receita/RT. Esses itens não foram ordenados automaticamente.</li>}
      <li><strong>{automaticProducts.length + (pending.length ? 4 : 3)}.</strong> Complete com água até <strong>{format(finalLiters, 1)} L de volume final</strong>, mantendo a agitação.</li>
      <li><strong>{automaticProducts.length + (pending.length ? 5 : 4)}.</strong> Confira visualmente a homogeneidade e mantenha a agitação conforme orientação dos produtos durante o uso.</li>
    </ol>
    {!sequenceReady && <p className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-xs font-bold leading-5 text-amber-950">Receita parcial: existe produto sem formulação automática. Não inicie a mistura até confirmar esse item.</p>}
  </div>;
}
...
function NumberInput({ label, value, onChange, suffix, placeholder }: { label: string; value: string; onChange: (value: string) => void; suffix: string; placeholder?: string }) {
  return <label className="grid gap-1.5 text-sm font-bold text-slate-700">{label}<div className="flex min-h-12 overflow-hidden rounded-xl border border-slate-200 bg-white focus-within:border-emerald-500"><input inputMode="decimal" type="text" className="min-w-0 flex-1 px-3 text-[16px] font-black text-slate-950 outline-none placeholder:font-bold placeholder:text-slate-400" value={value} placeholder={placeholder} onChange={(e) => onChange(e.target.value.replace(/[^0-9,.]/g, ""))}/>{suffix && <span className="grid shrink-0 place-items-center border-l border-slate-200 bg-slate-50 px-3 text-xs font-black text-slate-500">{suffix}</span>}</div></label>;
}
