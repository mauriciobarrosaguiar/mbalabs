...
export function DroneTeamMode(){
  ...
  return <main className="min-h-screen bg-[#f4f8f1] px-3 pb-28 pt-4 text-[#143d31] sm:px-6 sm:py-8"><div className="mx-auto grid w-full max-w-3xl gap-4">
    ...
    {loading?<div className="grid min-h-40 place-items-center rounded-2xl bg-white"><Loader2 className="animate-spin text-[#087a55]"/></div>:<>
      ...
      <section className={`rounded-[26px] border p-5 ${mode==="solo"?"border-emerald-200 bg-emerald-50":"border-[#dce7df] bg-white"}`}><div className="flex items-start gap-3"><CheckCircle2 size={21} className="mt-0.5 shrink-0 text-[#087a55]"/><div><strong className="block">{mode==="solo"?"Modo solo ativo":"Modo equipe ativo"}</strong><p className="mt-1 text-sm leading-5 text-[#61746a]">{mode==="solo"?"Você não precisa cadastrar pilotos. Suas operações continuam normalmente.":"Cadastre o piloto e crie o acesso dele sem sair do DroneGestor."}</p></div></div>{mode==="equipe"&&<Link href="/apps/dronegestor/pilotos" className="mt-4 flex min-h-12 items-center justify-center rounded-xl bg-[#087a55] px-4 text-sm font-black text-white no-underline">Cadastrar ou gerenciar pilotos</Link>}</section>
