...
import { ArrowLeft, CheckCircle2, FileText, FolderCheck, MapPinned, PlaneTakeoff, Printer, TriangleAlert } from "lucide-react";
  ...
  return <main className="min-h-screen bg-[#f4f8f1] px-3 pb-28 pt-4 text-[#143d31] sm:px-6 sm:py-8">
    <div className="mx-auto w-full max-w-5xl">
      ...
      {!mission.ordemServicoId ? <section className="mt-5 rounded-3xl border border-amber-200 bg-amber-50 p-5"><TriangleAlert className="mr-2 inline" size={18}/><strong>Nenhuma OS ativa foi encontrada.</strong><p className="mt-2 text-sm">Retorne a Operação, selecione ou retome a OS e volte aqui.</p></section> : <>
        ...
        <section className={`mt-4 rounded-3xl border p-5 ${pending.length ? "border-amber-200 bg-amber-50" : "border-emerald-200 bg-emerald-50"}`}>
          <div className="flex items-start gap-3">{pending.length ? <TriangleAlert className="shrink-0 text-amber-700"/> : <CheckCircle2 className="shrink-0 text-emerald-700"/>}<div><strong className="block">{pending.length ? "Ainda existem pendências" : "Pacote básico conferido"}</strong><p className="mt-1 text-sm">{pending.length ? "Regularize os itens abaixo antes do fechamento definitivo." : "Os itens básicos desta tela estão completos."}</p></div></div>
          {pending.length > 0 && <div className="mt-3 grid gap-2">{pending.map(item => <div key={item} className="rounded-xl bg-white px-3 py-2 text-sm font-bold">• {item}</div>)}</div>}
        </section>
