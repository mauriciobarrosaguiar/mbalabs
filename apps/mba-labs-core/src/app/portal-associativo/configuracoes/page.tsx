  ...
  return (
    <PortalAssociativoShell activePath="/portal-associativo/configuracoes" can={(section) => canPortalAccess(data.perfil, section)} companyName={data.companyName} roleLabel={data.perfilLabel} userName={data.current.usuario.nome}>
      <section className="grid gap-6">
        <PageHeader eyebrow="Portal Associativo" title="Configurações" description="Defina dados públicos, padrões de mensalidade, PIX e parâmetros de webhooks." actions={<BackButton href="/portal-associativo" />} />
        <MessageBanner ok={firstParam(params.ok)} error={firstParam(params.error) ?? data.error ?? undefined} />

        <form action={savePortalConfiguracoes}>
          <ResourceForm title="Dados da entidade" actions={<SubmitButton>Salvar configurações</SubmitButton>}>
            ...
            <FormSelect label="Tipo padrão de chácara/lote" name="tipo_unidade_padrao" defaultValue={String(data.configuracoes.tipo_unidade_padrao ?? "chacara")} options={PORTAL_UNIDADE_OPTIONS} />
            ...
            <FormTextarea label="Subtítulo" name="subtitulo" defaultValue={String(data.configuracoes.subtitulo ?? "")} />
          </ResourceForm>
        </form>

        <form action={savePortalConfiguracoesPagamento}>
          <ResourceForm title="Configurações de pagamento" actions={<SubmitButton>Salvar pagamento</SubmitButton>}>
            <FormSelect
            ...
            <FormInput label="Modo padrão de mensalidade" name="modo_cobranca_padrao" defaultValue={String(data.pagamento.modo_cobranca_padrao ?? "manual")} />
        ...
        <div className="panel p-5">
          <h2 className="text-lg font-semibold">Segredos de integração</h2>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Client ID, client secret, certificados, tokens e service role devem ficar em variáveis de ambiente da Vercel
            ou criptografados em `assoc_segredos_pagamento`. Eles não são exibidos no frontend.
          </p>
        </div>
      </section>
