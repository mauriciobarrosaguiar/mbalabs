...
export default async function PortalAssociativoPage() {
  ...
  const primaryMetrics = [
    ...
    { label: "Comprovantes", value: dashboard.metrics.comprovantesPendentes, href: "/portal-associativo/financeiro?status=aguardando_aprovacao" },
  ...
  return (
    ...
    >
      <section className="grid gap-4 sm:gap-5">
        <PageHeader eyebrow="Portal Associativo" title="Início" description="Escolha uma tarefa e acompanhe apenas o que precisa de atenção." />
        ...
        {onboarding.shouldShow && dashboard.perfil !== "portaria" ? (
          <details className="rounded-2xl border border-border bg-card p-4 shadow-sm">
            <summary className="cursor-pointer text-sm font-black">Implantação guiada · {onboarding.completed}/{onboarding.total} passos</summary>
            <section className="mt-4 grid gap-4">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <p className="eyebrow">Começar aqui</p>
                  <h2 className="text-xl font-black">Prepare o portal</h2>
                </div>
                <strong className="rounded-lg bg-primary px-3 py-2 text-sm text-primary-foreground">{onboarding.completed}/{onboarding.total} passos</strong>
              </div>
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {onboarding.steps.map((step) => (
                  <article className="grid gap-3 rounded-xl border border-border bg-muted/40 p-3" key={step.id}>
                    <div className="flex items-start justify-between gap-3">
                      <h3 className="font-black">{step.title}</h3>
                      <span className={step.done ? "rounded-full bg-emerald-100 px-2 py-1 text-xs font-bold text-emerald-700" : "rounded-full bg-amber-100 px-2 py-1 text-xs font-bold text-amber-700"}>{step.done ? "Pronto" : "Pendente"}</span>
                    </div>
                    <Link className={step.done ? "button-secondary w-fit" : "button-primary w-fit"} href={step.href === "/portal-associativo/configuracoes" ? "/portal-associativo/implantacao" : step.href}>{step.action}</Link>
                  </article>
                ))}
              </div>
            </section>
        ...
        <details className="rounded-2xl border border-border bg-card p-4 shadow-sm">
          <summary className="cursor-pointer text-sm font-black">Ver mais indicadores</summary>
          ...
          {dashboard.alertas.length ? (
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {dashboard.alertas.map((alerta) => (
                <Link className="rounded-xl border border-border bg-muted/40 p-3 transition hover:border-primary/60" href={alerta.href} key={alerta.title}>
                  <strong className="block text-sm">{alerta.title}</strong>
                  <span className="mt-1 block text-sm leading-5 text-muted-foreground">{alerta.detail}</span>
        ...
        <details className="rounded-2xl border border-border bg-card p-4 shadow-sm">
          <summary className="cursor-pointer text-sm font-black">Ver inadimplência por unidade e responsável</summary>
          <div className="mt-4 grid gap-4 xl:grid-cols-3">
            {dashboard.metrics.totalLoteamentos > 0 ? <Panel title="Por grupo/condomínio"><MiniRanking rows={dashboard.inadimplenciaPorLoteamento} /></Panel> : null}
            <Panel title="Por unidade"><MiniRanking rows={dashboard.inadimplenciaPorUnidade} /></Panel>
            <Panel title="Por responsável"><MiniRanking rows={dashboard.inadimplenciaPorResponsavel} /></Panel>
          </div>
        </details>

        <details className="rounded-2xl border border-border bg-card p-4 shadow-sm">
          <summary className="cursor-pointer text-sm font-black">Ver últimas cobranças</summary>
          <div className="mt-4">
            <DataTable
              columns={[
        ...
        <details className="rounded-2xl border border-border bg-card p-4 shadow-sm">
          <summary className="cursor-pointer text-sm font-black">Ver últimos pagamentos e cadastros</summary>
        ...
        <details className="rounded-2xl border border-border bg-card p-4 shadow-sm">
          <summary className="cursor-pointer text-sm font-black">Ver histórico recente</summary>
          <div className="mt-4">
            <DataTable
              columns={[
                { key: "acao", label: "Ação" },
                { key: "entidade", label: "Área" },
                { key: "criado_em", label: "Criado em" }
              ]}
...
function CompactMetricCard({ label, value, href }: { label: string | number; value: string | number; href?: string }) {
  const content = <><div className="text-xl font-black tracking-tight text-[#08111f] sm:text-2xl">{value}</div><div className="mt-1 text-[11px] font-black uppercase tracking-wide text-[#687385] sm:text-xs">{label}</div></>;
  ...
  return <article className="rounded-2xl border border-[#dfe6f0] bg-white px-4 py-4 shadow-[0_8px_24px_rgba(8,17,31,0.045)]">{content}</article>;
}
