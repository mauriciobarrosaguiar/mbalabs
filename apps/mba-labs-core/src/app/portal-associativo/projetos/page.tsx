...
import { BackButton, DataTable, FormCheckbox, FormInput, FormMoneyInput, FormSelect, FormTextarea, MessageBanner, PageHeader, ResourceForm, SubmitButton, formatMoney } from "@/components/ui-kit";
  ...
  return (
    <PortalAssociativoShell activePath="/portal-associativo/projetos" can={(section) => canPortalAccess(data.perfil, section)} companyName={data.companyName} roleLabel={data.perfilLabel} userName={data.current.usuario.nome}>
      <section className="grid gap-6">
        <PageHeader eyebrow="Portal Associativo" title="Projetos" description="Acompanhe projetos com valor previsto, arrecadado, percentual e documentos publicos." actions={<BackButton href="/portal-associativo" />} />
        ...
        {canWrite ? (
          <form action={savePortalProjeto}>
            ...
            >
              ...
              <FormInput label="Relatorio URL" name="relatorio_url" defaultValue={String(editing?.relatorio_url ?? "")} />
        ...
        <DataTable
          ...
          actions={(row) => (
            <div className="flex flex-wrap justify-end gap-2">
              {row.relatorio_url ? <Link className="button-secondary" href={String(row.relatorio_url)} target="_blank">Relatorio</Link> : null}
