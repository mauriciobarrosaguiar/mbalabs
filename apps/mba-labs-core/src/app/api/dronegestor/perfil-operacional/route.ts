...
function sanitize(value:unknown):Profile{const source=value&&typeof value==="object"&&!Array.isArray(value)?value as Record<string,unknown>:{};return{
  operadorNome:text(source.operadorNome,180),cpfCnpj:text(source.cpfCnpj,32),registroMapa:text(source.registroMapa,80),processoSei:text(source.processoSei,100),
