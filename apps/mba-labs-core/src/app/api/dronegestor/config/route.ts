...
export async function GET() {
  try {
    ...
    const settings = data?.detalhes && typeof data.detalhes === "object" && "settings" in data.detalhes
      ? sanitize((data.detalhes as any).settings)
      : defaults;

    return NextResponse.json({ ok: true, settings, canManage: current.canManage, updatedAt: data?.created_at ?? null });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Falha ao carregar configuração." }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (access.response) return access.response;
    const current = access.current!;
    if (!current.canManage) return NextResponse.json({ ok: false, error: "Somente administrador da empresa ou responsável técnico pode alterar os padrões." }, { status: 403 });
    ...
    return NextResponse.json({ ok: true, settings, updatedAt: now });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Falha ao salvar configuração." }, { status: 500 });
  }
}
