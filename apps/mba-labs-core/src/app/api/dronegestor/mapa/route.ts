...
export async function GET(request: NextRequest) {
  try {
    ...
    const row = (data ?? []).find((item: any) => !osId || text(item?.detalhes?.ordemServicoId, 100) === osId);
    if (!row) return NextResponse.json({ ok: true, evidence: null });

    return NextResponse.json({ ok: true, evidence: await signedEvidence(admin, row) });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const storagePath = `${owner}/${safePart(current.userId)}/${safePart(ordemServicoId || "sem-os")}/${Date.now()}-${evidenceId}.${image.extension}`;
    const admin = createSupabaseAdminClient() as any;
    ...
    const inserted = await admin.from("core_logs").insert({
      empresa_id: current.empresaId,
      usuario_id: current.userId,
      app_slug: "dronegestor",
      acao: ACTION,
      detalhes: details
    }).select("id,detalhes,created_at").single();
    ...
    const evidence = await signedEvidence(admin, inserted.data);
