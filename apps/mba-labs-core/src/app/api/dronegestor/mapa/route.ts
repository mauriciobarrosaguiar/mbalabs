...
function safePart(value: string) { return value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9_-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80) || "sem-os"; }
...
function detectImage(bytes: Buffer) {
  if (bytes.length >= 4 && bytes[0] === 0xff && bytes[1] === 0xd8 && bytes[2] === 0xff) return { extension: "jpg", contentType: "image/jpeg" };
...
function scopeQuery(query: any, current: Context) { return current.empresaId ? query.eq("empresa_id", current.empresaId) : query.eq("usuario_id", current.userId); }
...
async function signedEvidence(admin: any, row: any) {
  ...
  return { id: text(details.evidenceId, 100) || String(row.id), storagePath, url: signed.data.signedUrl, uploadedAt: text(details.uploadedAt, 60) || row.created_at, source: text(details.source, 20), ordemServicoId: text(details.ordemServicoId, 100), ordemServicoNumero: text(details.ordemServicoNumero, 100), talhaoNome: text(details.talhaoNome, 180), fazendaNome: text(details.fazendaNome, 180) };
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = admin.from("core_logs").select("id,detalhes,created_at").eq("app_slug", "dronegestor").eq("acao", ACTION).contains("detalhes", { ordemServicoId: osId }).order("created_at", { ascending: false }).limit(1);
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Falha ao carregar a foto do mapa." }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ordemServicoId) return NextResponse.json({ ok: false, error: "Selecione uma OS antes de enviar o mapa usado no voo." }, { status: 400 });
    if (!(file instanceof File) || file.size <= 0) return NextResponse.json({ ok: false, error: "Tire uma foto ou escolha uma imagem do mapa." }, { status: 400 });
    if (file.size > MAX_BYTES) return NextResponse.json({ ok: false, error: "A imagem é muito grande. Use uma foto de até 8 MB." }, { status: 400 });
    const bytes = Buffer.from(await file.arrayBuffer());
    const image = detectImage(bytes);
    if (!image) return NextResponse.json({ ok: false, error: "Arquivo inválido. Envie uma imagem JPG, PNG ou WebP." }, { status: 400 });
    ...
    const owner = current.empresaId ? `empresa-${safePart(current.empresaId)}` : `usuario-${safePart(current.userId)}`;
    const storagePath = `${owner}/${safePart(current.userId)}/${safePart(ordemServicoId)}/${Date.now()}-${evidenceId}.${image.extension}`;
    const admin = createSupabaseAdminClient() as any;
    const upload = await admin.storage.from(BUCKET).upload(storagePath, bytes, { contentType: image.contentType, upsert: false, cacheControl: "3600" });
    if (upload.error) throw upload.error;
    const uploadedAt = new Date().toISOString();
    const details = { evidenceId, storagePath, uploadedAt, source, ordemServicoId, ordemServicoNumero, talhaoNome, fazendaNome, originalBytes: bytes.length, contentType: image.contentType, piloto: current.userName };
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Falha ao salvar a foto do mapa." }, { status: 500 });
  }
}
