    ...
    <section className="overflow-hidden rounded-[28px] border border-[#123d2d]/10 bg-white shadow-[0_18px_55px_rgba(18,61,45,.10)]">
      <div className="border-b border-slate-100 px-4 py-5 sm:px-7">
        <div className="flex items-start gap-3">
          <div className="grid size-11 shrink-0 place-items-center rounded-[14px] bg-emerald-50 text-[#176445]">
        ...
        <div className="mt-4 flex items-start gap-3 rounded-2xl border border-[#e5c77d] bg-[#fff4d8] p-4 text-[#573d18]">
      ...
      <form action={registerPublicElshadayMember} className="grid min-w-0 gap-3.5 px-4 py-5 sm:grid-cols-2 sm:px-7 sm:py-6">
        ...
        <label className="grid min-w-0 gap-1.5 text-sm font-black text-slate-700 sm:col-span-2">
          Observação
          <textarea
            className="min-h-20 w-full min-w-0 max-w-full resize-y rounded-2xl border border-slate-300 bg-[#f3f6f3] px-4 py-3 text-base text-slate-950 outline-none placeholder:text-slate-500 focus:border-[#176445] focus:ring-4 focus:ring-emerald-900/10"
            name="observacoes"
            placeholder="Digite alguma observação (opcional)"
          />
        </label>

        <label className="flex min-w-0 items-start gap-3 rounded-2xl border border-emerald-800/30 bg-[#173f30] p-4 text-sm font-semibold leading-6 text-white sm:col-span-2">
        ...
        <div className="pt-1 sm:col-span-2">
          <ElshadaySubmitButton
            className="inline-flex min-h-13 w-full touch-manipulation items-center justify-center rounded-2xl bg-[#176445] px-6 text-base font-black text-white shadow-[0_10px_24px_rgba(23,100,69,.24)] transition active:scale-[.99]"
...
function inputClass(withIcon = false) {
  return "h-[50px] w-full min-w-0 max-w-full rounded-2xl border border-slate-300 bg-[#f3f6f3] " +
    (withIcon ? "pl-11 pr-4 " : "px-4 ") +
    "text-base text-slate-950 outline-none placeholder:text-slate-500 focus:border-[#176445] focus:ring-4 focus:ring-emerald-900/10";
}
...
function Field({
  ...
  return (
    <label className={"grid min-w-0 gap-1.5 text-sm font-black text-slate-700" + (wide ? " sm:col-span-2" : "")}>
      {label}
      <span className="relative block min-w-0">
        {Icon ? <Icon aria-hidden className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[#176445]" size={18} /> : null}
        <input
          {...props}
...
function PasswordField({
  ...
  return (
    <label className="grid min-w-0 gap-1.5 text-sm font-black text-slate-700">
      {label}
      <span className="relative block min-w-0">
        <LockKeyhole aria-hidden className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[#176445]" size={18} />
        <input
