...
export default async function CotacoesFinalizadasPage({
  searchParams
}: {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
}) {
  const params = await searchParams;
  const { rows, error } = await listCotacoes("finalizada");
    ...
    <main>
      ...
      <section className="page-shell grid gap-6 py-8">
        <PageHeader
          ...
          description="Consulte o histórico de cotações encerradas."
