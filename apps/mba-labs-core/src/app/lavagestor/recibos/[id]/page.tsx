...
function receiptImageData(recibo: Recibo): ReceiptImageData { return { id: recibo.id, numero: recibo.numero, empresaNome: recibo.empresa.nome, empresaRazao: recibo.empresa.razao_social ?? undefined, empresaInfo: [recibo.empresa.cnpj, recibo.empresa.telefone, recibo.empresa.cidade_uf, recibo.empresa.endereco].filter(Boolean).join(" - "), corPrincipal: recibo.empresa.cor_principal ?? undefined, cliente: recibo.cliente, whatsapp: recibo.whatsapp || "Não informado", veiculo: recibo.veiculo, lavador: recibo.funcionario, entrada: formatDateTime(recibo.data_entrada), finalizacao: formatDateTime(recibo.data_finalizacao), pagamento: paymentLabel(recibo), entrega: deliveryLabel(recibo), servicos: recibo.servicos.map((servico) => ({ descricao: servico.descricao, valor: formatMoney(servico.valor) })), totalBruto: formatMoney(recibo.valor_total), desconto: formatMoney(recibo.valor_desconto), totalFinal: formatMoney(recibo.valor_final), valorRecebido: formatMoney(recibo.valor_recebido), valorPendente: formatMoney(recibo.valor_pendente), pagamentos: recibo.pagamentos.map((pagamento) => `${formatMoney(pagamento.valor)} ${pagamento.forma_pagamento} - ${formatDateTime(pagamento.data_pagamento)}`) }; }
