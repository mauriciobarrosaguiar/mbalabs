    ...
    <LavaGestorShell activePath="/lavagestor/fila" companyName={recibo.empresa.nome}>
      ...
      <section className="grid max-w-full gap-6 overflow-x-hidden">
        ...
        <article className="receipt-print mx-auto grid w-full max-w-3xl gap-4 rounded-xl border border-border bg-white p-5 text-[#10201a] shadow-sm">
          <header className="grid gap-3 border-b border-border pb-3 sm:grid-cols-[1fr_auto]"><div>{recibo.empresa.logo_url ? <img className="mb-2 max-h-16 max-w-40 object-contain" src={recibo.empresa.logo_url} alt={recibo.empresa.nome} /> : null}<p className="text-xs font-black uppercase tracking-[0.14em]" style={{ color: recibo.empresa.cor_principal }}>Recibo de serviço</p><h2 className="mt-1 text-2xl font-black">{recibo.empresa.nome}</h2>{recibo.empresa.razao_social ? <p className="text-sm font-semibold text-slate-600">{recibo.empresa.razao_social}</p> : null}<p className="text-sm text-slate-600">{[recibo.empresa.cnpj, recibo.empresa.telefone, recibo.empresa.cidade_uf].filter(Boolean).join(" · ")}</p>{recibo.empresa.endereco ? <p className="text-sm text-slate-600">{recibo.empresa.endereco}</p> : null}</div><div className="rounded-lg bg-emerald-50 p-3 text-left sm:text-right"><p className="text-xs font-black uppercase text-emerald-800">Nº</p><p className="text-xl font-black">{recibo.numero}</p><p className="text-xs font-semibold text-slate-600">{formatDateTime(recibo.data_entrada)}</p></div></header>
          <section className="grid gap-2 sm:grid-cols-2"><Info label="Cliente" value={recibo.cliente} /><Info label="WhatsApp" value={recibo.whatsapp || "Não informado"} /><Info label="Veículo / item" value={recibo.veiculo} /><Info label="Lavador" value={recibo.funcionario} /><Info label="Entrada" value={formatDateTime(recibo.data_entrada)} /><Info label="Finalização" value={formatDateTime(recibo.data_finalizacao)} /><Info label="Pagamento" value={paymentLabel(recibo)} /><Info label="Entrega" value={deliveryLabel(recibo)} /></section>
          <section className="grid gap-2"><h3 className="text-sm font-black uppercase tracking-[0.12em] text-slate-500">Serviços</h3><div className="grid gap-2">{recibo.servicos.length === 0 ? <p className="rounded-lg bg-slate-50 p-2 text-sm font-semibold text-slate-600">Nenhum serviço detalhado.</p> : recibo.servicos.map((servico) => <div className="flex items-center justify-between gap-3 rounded-lg border border-border p-2" key={servico.id}><span className="font-semibold">{servico.descricao}</span><strong>{formatMoney(servico.valor)}</strong></div>)}</div></section>
          <section className="grid gap-1 rounded-lg bg-slate-50 p-3"><MoneyLine label="Total bruto" value={recibo.valor_total} /><MoneyLine label="Desconto" value={recibo.valor_desconto} /><MoneyLine label="Total final" value={recibo.valor_final} strong /><MoneyLine label="Valor recebido" value={recibo.valor_recebido} /><MoneyLine label="Valor pendente" value={recibo.valor_pendente} /></section>
          ...
          <footer className="grid gap-1 border-t border-border pt-3 text-center text-xs font-semibold text-slate-500"><p>Obrigado pela preferência.</p><p>Recibo gerado pelo LavaGestor · {recibo.empresa.nome}</p></footer>
        </article>
