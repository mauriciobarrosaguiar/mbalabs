    ...
    <LavaGestorShell activePath="/lavagestor/fila">
      <section className="grid gap-6">
        <PageHeader
          ...
          description="Comprovante da lavagem para enviar ao cliente ou imprimir."
          actions={
            <>
              <BackButton href="/lavagestor/fila" />
              <PrintButton />
              {recibo.whatsapp ? (
                <a className="button-primary print:hidden" href={whatsappReciboLink(recibo)} target="_blank" rel="noreferrer">
                  Enviar no WhatsApp
                </a>
              ) : null}
            </>
        ...
        <article className="mx-auto grid w-full max-w-3xl gap-5 rounded-xl border border-border bg-white p-5 text-[#10201a] shadow-sm print:border-0 print:shadow-none">
          <header className="grid gap-3 border-b border-border pb-4 sm:grid-cols-[1fr_auto]">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] text-emerald-700">Recibo de serviço</p>
            ...
            <div className="rounded-lg bg-emerald-50 p-3 text-left sm:text-right">
              <p className="text-xs font-black uppercase text-emerald-800">Nº</p>
          ...
          <section className="grid gap-3 sm:grid-cols-2">
            ...
            <Info label="Finalização" value={formatDateTime(recibo.data_finalizacao)} />
          ...
          <section className="grid gap-2">
            ...
            <div className="grid gap-2">
              {recibo.servicos.length === 0 ? (
                <p className="rounded-lg bg-slate-50 p-3 text-sm font-semibold text-slate-600">Nenhum serviço detalhado.</p>
              ) : (
                recibo.servicos.map((servico) => (
                  <div className="flex items-center justify-between gap-3 rounded-lg border border-border p-3" key={servico.id}>
          ...
          <section className="grid gap-2 rounded-lg bg-slate-50 p-4">
          ...
          {recibo.pagamentos.length > 0 ? (
            <section className="grid gap-2">
              <h3 className="text-sm font-black uppercase tracking-[0.12em] text-slate-500">Pagamentos</h3>
              {recibo.pagamentos.map((pagamento) => (
                <div className="rounded-lg border border-border p-3 text-sm" key={pagamento.id}>
                  <strong>{formatMoney(pagamento.valor)}</strong>
                  <span className="ml-2 text-slate-600">{pagamento.forma_pagamento} · {formatDateTime(pagamento.data_pagamento)}</span>
                </div>
              ))}
            </section>
          ) : null}
          ...
          <footer className="grid gap-3 border-t border-border pt-4 text-center text-xs font-semibold text-slate-500">
            <p>Obrigado pela preferência.</p>
            <p>Recibo gerado pelo LavaGestor · MBA Labs</p>
          </footer>
        </article>

        {recibo.status !== "entregue" ? (
          <form action={updateLavagemStatus} className="mx-auto grid w-full max-w-3xl gap-2 rounded-lg border border-emerald-200 bg-emerald-50 p-3 print:hidden">
            <input name="id" type="hidden" value={recibo.id} />
            <input name="acao" type="hidden" value="entregar" />
            <input name="return_to" type="hidden" value={`/lavagestor/recibos/${recibo.id}`} />
            <button className="button-primary" type="submit">
              {recibo.entrega_tipo === "levar" ? "Marcar entregue ao cliente" : "Marcar veículo retirado"}
            </button>
          </form>
        ) : (
          <div className="mx-auto w-full max-w-3xl rounded-lg border border-emerald-300 bg-emerald-50 p-3 text-sm font-black text-emerald-950 print:hidden">
            Entrega concluída em {formatDateTime(recibo.data_entrega)}.
          </div>
        )}
      </section>
    </LavaGestorShell>
...
function MoneyLine({ label, value, strong = false }: { label: string; value: number; strong?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <strong className={strong ? "text-xl" : ""}>{formatMoney(value)}</strong>
    </div>
  );
}

function paymentLabel(recibo: Recibo) {
  const status = recibo.status_pagamento === "pago" ? "Pago" : recibo.status_pagamento === "fiado" ? "Fiado" : recibo.status_pagamento === "parcial" ? "Parcial" : "Aberto";
  return [status, recibo.forma_pagamento].filter(Boolean).join(" · ");
}

function deliveryLabel(recibo: Recibo) {
  if (recibo.entrega_tipo === "levar") {
    return recibo.endereco_entrega ? `Levar ao cliente: ${recibo.endereco_entrega}` : "Levar ao cliente";
  }
  return "Cliente retira";
}

function whatsappReciboLink(recibo: Recibo) {
  const phone = String(recibo.whatsapp ?? "").replace(/\D/g, "");
  const servicos = recibo.servicos.map((item) => `- ${item.descricao}: ${formatMoney(item.valor)}`).join("\n");
  const texto = `Olá, ${recibo.cliente}! Segue o recibo da lavagem.\n\nRecibo: ${recibo.numero}\nVeículo/item: ${recibo.veiculo}\nServiços:\n${servicos || "-"}\n\nTotal: ${formatMoney(recibo.valor_final)}\nPagamento: ${paymentLabel(recibo)}\nEntrega: ${deliveryLabel(recibo)}\n\nObrigado pela preferência!`;
