      ...
      <section className="grid gap-6">
        <div className="report-no-print">
          <PageHeader
            eyebrow="LavaGestor"
            title="Relatórios"
            description="Fechamento completo por páginas: resumo, análises, lavagens, comissões e vales."
        ...
        <article className="report-print grid gap-6 rounded-xl border border-border bg-white p-5 shadow-sm">
...
function ReportPage({ title, companyName, filters, generatedAt, page, children }: { title: string; companyName: string; filters: { inicio: string; fim: string }; generatedAt?: string; page: string; children: React.ReactNode }) {
  return <section className="report-sheet grid gap-4"><header className="border-b border-border pb-3"><div className="flex items-start justify-between gap-3"><p className="text-sm font-black uppercase tracking-[0.14em] text-emerald-700">LavaGestor</p><p className="text-xs font-semibold text-muted-foreground">Relatório do período - MBA Labs</p></div><h2 className="mt-6 text-3xl font-black">{title}</h2><p className="text-sm font-semibold text-muted-foreground">{companyName} · {formatDate(filters.inicio)} até {formatDate(filters.fim)}{generatedAt ? ` · Gerado em ${formatDateTime(generatedAt)}` : ""}</p></header>{children}<footer className="mt-auto grid grid-cols-3 border-t border-border pt-3 text-xs font-semibold text-muted-foreground"><span>Gerado pelo LavaGestor - MBA Labs</span><span className="text-center">Relatório financeiro</span><span className="text-right">Página {page}</span></footer></section>;
}
...
function Card({ label, value, helper, green, yellow }: { label: string; value: string | number; helper: string; green?: boolean; yellow?: boolean }) {
  ...
  return <div className={`rounded-sm border p-3 ${tone}`}><p className="text-xs font-black uppercase tracking-[0.08em] text-muted-foreground">{label}</p><strong className="mt-3 block text-2xl font-black">{value}</strong><p className="mt-2 text-xs font-semibold text-muted-foreground">{helper}</p></div>;
}
...
function Table({ headers, rows }: { headers: string[]; rows: Array<Array<string | number>> }) {
  return <div className="w-full overflow-x-auto rounded-sm border border-border"><table className="w-full min-w-[680px] border-collapse text-left text-xs"><thead><tr className="bg-emerald-700 text-white">{headers.map((header) => <th className="border border-emerald-600 px-2 py-2 font-black" key={header}>{header}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr className={index === rows.length - 1 && rows.length > 1 ? "bg-emerald-50 font-bold" : index % 2 ? "bg-slate-50" : "bg-white"} key={index}>{row.map((cell, cellIndex) => <td className={`border border-border px-2 py-2 ${cellIndex > 1 ? "text-right" : ""}`} key={`${index}-${cellIndex}`}>{cell}</td>)}</tr>)}</tbody></table></div>;
}

function ValeHistory({ vale }: { vale: AnyRow }) {
  const movimentos = Array.isArray(vale.movimentos) ? (vale.movimentos as AnyRow[]) : [];
  return <div className="rounded-sm border border-border p-3"><h4 className="text-sm font-black">{String(vale.funcionario || "Funcionário")} · vale de {formatMoney(vale.valor_original ?? vale.valor)}</h4><p className="text-xs font-semibold text-muted-foreground">Descontado: {formatMoney(vale.valor_descontado)} · Saldo para próxima vez: {formatMoney(vale.saldo_restante)}</p>{movimentos.length ? <Table headers={["Data", "Valor descontado", "Saldo antes", "Saldo depois", "Observação"]} rows={movimentos.map((mov) => [formatDateTime(mov.created_at), formatMoney(mov.valor_descontado), formatMoney(mov.saldo_antes), formatMoney(mov.saldo_depois), String(mov.observacao || "-")])} /> : <p className="mt-2 rounded-sm bg-amber-50 p-2 text-xs font-semibold text-amber-950">Sem movimento detalhado salvo ainda. O relatório mostra o total já descontado e o saldo restante do vale.</p>}</div>;
}
...
function Empty({ text }: { text: string }) { return <p className="rounded-sm bg-muted p-3 text-sm font-semibold text-muted-foreground">{text}</p>; }
...
@media print {
  ...
  .report-print { position: absolute !important; left: 0 !important; top: 0 !important; width: 100% !important; margin: 0 !important; padding: 0 !important; border: 0 !important; box-shadow: none !important; gap: 0 !important; font-size: 8pt !important; }
  .report-no-print { display: none !important; }
  .report-sheet { min-height: 276mm !important; page-break-after: always !important; break-after: page !important; padding: 0 !important; gap: 7px !important; }
  ...
  table { min-width: 0 !important; font-size: 7pt !important; }
