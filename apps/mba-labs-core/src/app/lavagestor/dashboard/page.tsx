...
export default async function LavaGestorPortalPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const { perfil } = await requireLavaGestorOwnerAccess("/lavagestor/dashboard");
  const [dashboard, { config }] = await Promise.all([getLavaDashboard(), getLavaConfiguracoesEmpresa("/lavagestor/dashboard")]);
  ...
  return (
    <LavaGestorShell activePath="/lavagestor/dashboard" companyName={companyName} userName={user.nome} roleLabel={roleLabel} perfil={perfil}>
