...
export default async function LavaGestorDashboardPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const params = await searchParams;
  await requireLavaGestorFinanceAccess("/lavagestor/dashboard");
  ...
  return (
    <LavaGestorShell activePath="/lavagestor/dashboard" companyName={companyName} userName={user.nome} roleLabel={roleLabel}>
      <Link className="fixed right-4 top-[4.25rem] z-20 rounded-full border border-emerald-200 bg-white px-3 py-2 text-xs font-black uppercase tracking-[0.08em] text-primary shadow-md lg:hidden" href="/lavagestor/operacao/fila">Fila</Link>
      <div className="space-y-5">
        <div className="space-y-4">
          ...
          <div className="grid gap-2 lg:grid-cols-[1fr_auto]">
            ...
            <Link className="button-primary min-h-11 justify-center rounded-xl text-base font-black" href="/lavagestor/operacao/entrada">Nova lavagem</Link>
          </div>
        </div>
