      ...
      <section className="grid gap-6">
        ...
        <div className="grid gap-4 xl:grid-cols-4">
          {groups.map((group) => {
            ...
            return (
              <section className="grid content-start gap-3 rounded-lg border border-border bg-white p-3 shadow-sm xl:max-h-[calc(100vh-260px)] xl:overflow-y-auto" key={group.title}>
                ...
                {items.length === 0 ? <p className="rounded-lg bg-muted p-4 text-sm text-muted-foreground">Nenhuma lavagem nesta etapa.</p> : items.map((row) => <FilaCard key={String(row.id)} row={row} />)}
              </section>
            );
          })}
        </div>
    ...
    <details className="group rounded-lg border border-border bg-[#fbfdfc] shadow-sm">
      <summary className="grid cursor-pointer list-none gap-3 p-3 [&::-webkit-details-marker]:hidden">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="truncate font-black" title={String(row.cliente || "Cliente")}>{String(row.cliente || "Cliente")}</h3>
            <p className="truncate text-xs font-semibold text-muted-foreground" title={String(row.veiculo || "-")}>{String(row.veiculo || "-")}</p>
          </div>
          <span className="shrink-0 rounded-full bg-[#dff7ec] px-2 py-1 text-xs font-black text-[#0f5132]">{String(row.status_label ?? status)}</span>
        </div>
      ...
      <div className="grid gap-4 border-t border-border p-3">
        ...
        <div className="grid gap-2">
          {status === "na_fila" ? <><StatusButton id={id} action="iniciar" label="Iniciar lavagem" /><CancelForm id={id} /></> : null}
          ...
          {status === "finalizado" || status === "aguardando_finalizacao" ? (
            <>
              {phone ? <a className="button-secondary" href={whatsappLink(row)} target="_blank" rel="noreferrer">Avisar cliente no WhatsApp</a> : <p className="rounded-lg bg-muted p-3 text-sm text-muted-foreground">WhatsApp não informado para este cliente.</p>}
              <StatusButton id={id} action="avisar_cliente" label="Marcar cliente avisado" />
              <Link className="button-primary" href={`/lavagestor/pagamentos?lavagem=${id}`}>Registrar pagamento</Link>
              <Link className="button-secondary" href={`/lavagestor/recibos/${id}`}>Ver recibo</Link>
            </>
          ) : null}
          ...
          {status === "cliente_avisado" || status === "pago" ? (
            <>
              ...
              <Link className="button-secondary" href={`/lavagestor/recibos/${id}`}>Ver recibo</Link>
              <StatusButton id={id} action="entregar" label={row.entrega_tipo === "levar" ? "Marcar entregue ao cliente" : "Entregar veículo"} returnTo={`/lavagestor/recibos/${id}`} />
            </>
          ) : null}
        </div>
      </div>
...
function MiniInfo({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) {
  return <div className="min-w-0 rounded-lg bg-muted px-2 py-2"><dt className="font-black uppercase tracking-[0.08em]">{label}</dt><dd className={`mt-1 truncate ${strong ? "font-black text-foreground" : "font-semibold text-foreground"}`}>{value}</dd></div>;
}

function Info({ label, value }: { label: string; value: string }) {
  return <div><dt className="text-xs font-black uppercase tracking-[0.1em] text-muted-foreground">{label}</dt><dd className="mt-1 font-semibold">{value}</dd></div>;
}

function StatusButton({ id, action, label, returnTo = "/lavagestor/fila" }: { id: string; action: string; label: string; returnTo?: string }) {
  return (
    <form action={updateLavagemStatus}>
      <input name="id" type="hidden" value={id} />
      <input name="acao" type="hidden" value={action} />
      <input name="return_to" type="hidden" value={returnTo} />
      <button className="button-primary w-full" type="submit">{label}</button>
    </form>
  );
}
...
function CancelForm({ id }: { id: string }) {
  return (
    <form action={updateLavagemStatus} className="grid gap-2 rounded-lg border border-red-200 bg-red-50 p-3">
      ...
      <label className="grid gap-2 text-sm font-bold text-red-900">Motivo do cancelamento<select className="input border-red-200 bg-white" name="motivo_cancelamento" required defaultValue=""><option value="">Selecione o motivo</option>{cancelReasons.map((reason) => <option key={reason} value={reason}>{reason}</option>)}</select></label>
      <button className="button-danger w-full" type="submit">Cancelar lavagem</button>
    </form>
  );
}

function MiniPaymentForm({ row }: { row: Record<string, unknown> }) {
  if (row.status_pagamento === "pago") return null;
  return (
    <form action={registrarPagamentoLavagem} className="grid gap-2">
      ...
      <select className="input" name="forma_pagamento" defaultValue="pix"><option value="dinheiro">Dinheiro</option><option value="pix">Pix</option><option value="cartao">Cartão</option><option value="fiado">Fiado</option></select>
      <button className="button-primary w-full" type="submit">Registrar pagamento</button>
    </form>
  );
}
...
function whatsappLink(row: Record<string, unknown>) {
  ...
  const entregaTexto = entregaTipo === "levar" ? `Nossa equipe vai levar até você.${endereco ? `\nEndereço/referência: ${endereco}` : ""}` : "Já pode retirar.";
...
function entregaInfo(row: Record<string, unknown>) {
  const entregaTipo = String(row.entrega_tipo ?? "retirar");
  if (entregaTipo !== "levar") return "Cliente irá retirar";
  const endereco = String(row.endereco_entrega ?? "").trim();
