...
import { CheckCircle2, GraduationCap, LayoutDashboard, LoaderCircle, LogOut, School, ShieldCheck, UserPlus } from "lucide-react";
...
function OwnerDashboard({schools}:{schools:SchoolRow[]}){return <><section className="grid gap-4 sm:grid-cols-3"><Metric label="Escolas cadastradas" value={String(schools.length)}/><Metric label="Nível de acesso" value="Proprietário"/><Metric label="Ambiente" value="Ativo"/></section><section className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6"><div className="flex flex-col justify-between gap-5 md:flex-row md:items-center"><div><p className="text-sm font-black uppercase tracking-[.14em] text-emerald-700">Central administrativa</p><h2 className="mt-2 text-2xl font-black">Painel do ADMIN MBA</h2><p className="mt-2 text-emerald-800">Escolas, perfis, planos, pagamentos e auditoria.</p></div><Link className="flex min-h-12 items-center gap-2 rounded-2xl bg-[#176b5b] px-5 font-black text-white" href="/mba-escola/admin"><LayoutDashboard size={20}/> Abrir painel</Link></div></section><section className="rounded-3xl border border-slate-200 bg-white p-6"><h2 className="text-2xl font-black">Escolas</h2><div className="mt-4 grid gap-2">{schools.map(s=><div className="flex justify-between rounded-xl border border-slate-200 p-4" key={s.id}><b>{s.nome}</b><span className="text-sm">{s.status}</span></div>)}</div></section></>;}
