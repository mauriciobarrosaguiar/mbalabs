...
type ContentRow = {
  id: string;
  table: "escola_comunicados" | "escola_atividades" | "escola_aulas" | "escola_acompanhamentos";
  type: string;
  title: string;
  status: string;
  schoolId: string;
...
export default function MbaEscolaAdminPage() {
  ...
  const load = useCallback(async (activeSession: Session) => {
    ...
    const merged: ContentRow[] = [
      ...((commRes.data ?? []) as any[]).map((item) => ({ id: item.id, table: "escola_comunicados" as const, type: "Comunicado", title: item.titulo, status: item.status, schoolId: item.escola_id, date: item.criado_em })),
      ...((activityRes.data ?? []) as any[]).map((item) => ({ id: item.id, table: "escola_atividades" as const, type: "Atividade", title: item.titulo, status: item.status, schoolId: item.escola_id, date: item.criado_em })),
      ...((classRes.data ?? []) as any[]).map((item) => ({ id: item.id, table: "escola_aulas" as const, type: "Aula", title: item.titulo, status: "registrada", schoolId: item.escola_id, date: item.criado_em })),
      ...((followRes.data ?? []) as any[]).map((item) => ({ id: item.id, table: "escola_acompanhamentos" as const, type: "Acompanhamento", title: item.titulo, status: item.status, schoolId: item.escola_id, date: item.criado_em }))
  ...
  const refresh = () => session && void load(session);
  ...
  async function removeSchool(id: string, nome: string) {
    if (!window.confirm(`Excluir a escola ${nome}? Esta ação também remove os dados vinculados.`)) return;
  ...
  async function createInvite(formData: FormData) {
    ...
    setMessage(error ? error.message : "Convite criado. No primeiro acesso, o usuário poderá definir a senha.");
  }
  ...
  return (
    <main className="min-h-screen bg-[#f5f8fb] pb-12 text-slate-900">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex min-h-20 w-[min(1280px,calc(100%-28px))] items-center justify-between gap-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div><p className="font-black">MBA Escola</p><p className="text-sm text-slate-500">ADMIN MBA · Proprietário</p></div>
          </div>
      ...
      <div className="mx-auto grid w-[min(1280px,calc(100%-28px))] gap-6 py-7">
        <section>
          <p className="text-sm font-black uppercase tracking-[.16em] text-[#176b5b]">Controle total</p>
          <h1 className="mt-2 text-3xl font-black sm:text-4xl">Painel do proprietário</h1>
          <p className="mt-2 max-w-3xl leading-7 text-slate-500">Escolas, usuários, publicações, planos, cobranças e auditoria em um único lugar.</p>
        </section>
  ...
  return (
    <div className="grid gap-6">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <p className="text-sm font-black text-[#176b5b]">Ambiente de testes</p>
        <h2 className="mt-1 text-2xl font-black">Acessos fake</h2>
        <p className="mt-2 text-sm text-slate-500">Todos usam a senha <strong className="text-slate-900">123456</strong>.</p>
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {testAccounts.map(([role, email]) => <div className="rounded-2xl border border-slate-200 p-4" key={email}><p className="font-black">{role}</p><p className="mt-1 break-all text-sm text-slate-500">{email}</p></div>)}
        </div>
      </section>
    </div>
  );
...
function SchoolsTab({ schools, createSchool, changeStatus, removeSchool }: { schools: SchoolRow[]; createSchool: (f: FormData) => Promise<void>; changeStatus: (id: string, status: string) => Promise<void>; removeSchool: (id: string, nome: string) => Promise<void> }) {
  return (
    <div className="grid gap-5 lg:grid-cols-[340px_1fr]">
      <FormCard title="Adicionar escola"><form action={createSchool} className="grid gap-3"><Input name="nome" placeholder="Nome da escola" /><Submit label="Criar escola" /></form></FormCard>
      <ListCard title="Escolas cadastradas">{schools.map((school) => <Row key={school.id} title={school.nome} subtitle={`Status: ${school.status}`}><select className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-bold" value={school.status} onChange={(e) => void changeStatus(school.id, e.target.value)}><option value="teste">Teste</option><option value="ativa">Ativa</option><option value="bloqueada">Bloqueada</option><option value="cancelada">Cancelada</option></select><IconButton label="Excluir" danger onClick={() => void removeSchool(school.id, school.nome)} icon={<Trash2 size={17} />} /></Row>)}</ListCard>
    </div>
  );
}
...
function ProfilesTab({ schools, profiles, createInvite, toggleProfile, removeProfile }: { schools: SchoolRow[]; profiles: ProfileRow[]; createInvite: (f: FormData) => Promise<void>; toggleProfile: (p: ProfileRow) => Promise<void>; removeProfile: (p: ProfileRow) => Promise<void> }) {
  return (
    <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
      <FormCard title="Adicionar perfil"><form action={createInvite} className="grid gap-3"><Input name="nome" placeholder="Nome" /><Input name="email" placeholder="E-mail" type="email" /><Select name="escola_id" options={schools.map((s) => [s.id, s.nome])} placeholder="Selecione a escola" /><Select name="papel" options={[["admin_escola","Admin da Escola"],["coordenacao","Coordenação"],["professor","Professor"],["responsavel","Responsável"],["aluno","Aluno"]]} placeholder="Selecione o cargo" /><Submit label="Criar convite" /></form></FormCard>
      <ListCard title="Perfis e acessos">{profiles.map((profile) => <Row key={profile.id} title={profile.nome} subtitle={`${roleLabel(profile.papel)} · ${profile.escola?.nome || "Escola"}${profile.is_teste ? " · TESTE" : ""}`}><span className={`rounded-full px-3 py-1 text-xs font-black ${profile.ativo ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{profile.ativo ? "Ativo" : "Inativo"}</span><IconButton label={profile.ativo ? "Inativar" : "Reativar"} onClick={() => void toggleProfile(profile)} icon={profile.ativo ? <Ban size={17} /> : <CheckCircle2 size={17} />} /><IconButton label="Excluir" danger onClick={() => void removeProfile(profile)} icon={<Trash2 size={17} />} /></Row>)}</ListCard>
    </div>
  );
}
...
function ContentTab({ content, schoolName, removeContent }: { content: ContentRow[]; schoolName: Map<string,string>; removeContent: (item: ContentRow) => Promise<void> }) {
  return <ListCard title="Tudo que foi publicado/registrado">{content.map((item) => <Row key={`${item.table}-${item.id}`} title={item.title} subtitle={`${item.type} · ${schoolName.get(item.schoolId) || "Escola"} · ${item.status} · ${formatDate(item.date)}`}><IconButton label="Excluir" danger onClick={() => void removeContent(item)} icon={<Trash2 size={17} />} /></Row>)}</ListCard>;
}
...
function PlansTab({ plans, createPlan, togglePlan }: { plans: PlanRow[]; createPlan: (f: FormData) => Promise<void>; togglePlan: (p: PlanRow) => Promise<void> }) {
  return (
    <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
      <FormCard title="Novo plano"><form action={createPlan} className="grid gap-3"><Input name="nome" placeholder="Nome do plano" /><Input name="descricao" placeholder="Descrição" /><Input name="preco_mensal" placeholder="Preço mensal" type="number" step="0.01" /><Input name="limite_alunos" placeholder="Limite de alunos" type="number" /><Input name="limite_usuarios" placeholder="Limite de usuários" type="number" /><Submit label="Criar plano" /></form></FormCard>
      <ListCard title="Planos">{plans.map((plan) => <Row key={plan.id} title={plan.nome} subtitle={`R$ ${Number(plan.preco_mensal).toFixed(2)} / mês · ${plan.limite_alunos ?? "∞"} alunos · ${plan.limite_usuarios ?? "∞"} usuários`}><IconButton label={plan.ativo ? "Inativar" : "Ativar"} onClick={() => void togglePlan(plan)} icon={plan.ativo ? <Ban size={17} /> : <CheckCircle2 size={17} />} /></Row>)}</ListCard>
    </div>
  );
}
...
function PaymentsTab({ schools, payments, createPayment, markPaid }: { schools: SchoolRow[]; payments: PaymentRow[]; createPayment: (f: FormData) => Promise<void>; markPaid: (p: PaymentRow) => Promise<void> }) {
  return (
    <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
      <FormCard title="Registrar cobrança"><form action={createPayment} className="grid gap-3"><Select name="escola_id" options={schools.map((s) => [s.id, s.nome])} placeholder="Selecione a escola" /><Input name="valor" placeholder="Valor" type="number" step="0.01" /><Input name="vencimento" type="date" /><Submit label="Registrar cobrança" /></form></FormCard>
      <ListCard title="Pagamentos">{payments.length ? payments.map((payment) => <Row key={payment.id} title={`${payment.escola?.nome || "Escola"} · R$ ${Number(payment.valor).toFixed(2)}`} subtitle={`${payment.status} · vencimento ${payment.vencimento ? formatDate(payment.vencimento) : "não informado"}`}><span className={`rounded-full px-3 py-1 text-xs font-black ${payment.status === "pago" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{payment.status}</span>{payment.status !== "pago" ? <IconButton label="Marcar pago" onClick={() => void markPaid(payment)} icon={<CheckCircle2 size={17} />} /> : null}</Row>) : <Empty text="Nenhuma cobrança registrada." />}</ListCard>
    </div>
  );
}
...
function Select({ name, options, placeholder }: { name: string; options: readonly (readonly [string,string])[]; placeholder: string }) { return <select name={name} required defaultValue="" className="min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm outline-none"><option value="" disabled>{placeholder}</option>{options.map(([value,label]) => <option value={value} key={value}>{label}</option>)}</select>; }
