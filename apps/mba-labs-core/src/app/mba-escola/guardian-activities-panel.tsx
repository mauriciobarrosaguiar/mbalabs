...
export default function GuardianActivitiesPanel({ supabase }: Props) {
  ...
  return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
    <div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-emerald-50 text-[#176b5b]"><ClipboardList size={20}/></div><div><h3 className="text-xl font-black">Atividades para casa</h3><p className="mt-1 text-sm text-slate-500">Informativos publicados pelos professores, prazos e situação das atividades de cada filho.</p></div></div>
    {error ? <p className="mt-4 rounded-xl bg-rose-50 p-3 text-sm font-bold text-rose-800">{error}</p> : null}
    {!rows.length ? <p className="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-slate-500">Nenhuma atividade para casa publicada para as turmas dos seus filhos.</p> : <div className="mt-5 grid gap-3">{rows.map(({ child, activity, delivery }) => <article className="rounded-2xl border border-slate-200 p-4" key={`${activity.id}-${child.id}`}>
