...
import type { PurchaseOrder, PurchaseOrderItem, PurchaseOrderItemFulfillmentStatus } from "@/modules/cotacoes/lib/types";
...
export function WinnerOrderForm({
  ...
  useEffect(() => {
    if (locked) return;
    void fetch(`/api/cotacoes/public-order/${order.publicToken}`, {
      method: "POST",
  ...
  async function persist(action: "save" | "finalize") {
    ...
    try {
      const response = await fetch(`/api/cotacoes/public-order/${order.publicToken}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action,
