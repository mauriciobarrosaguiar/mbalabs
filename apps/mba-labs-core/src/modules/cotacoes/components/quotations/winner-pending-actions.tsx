...
export function WinnerPendingActions({
  ...
  async function run(action: "send_next" | "new_quotation" | "resolve" | "cancel") {
    setLoading(action);
    try {
      const response = await fetch(`/api/cotacoes/winner-pendencies/${quotationId}`, {
        method: "POST",
      ...
      if (action === "new_quotation" && payload.quotation?.id) {
        router.push(`/cotacoes/${moduleType === "bidding" ? "licitacoes" : "cotacoes-farmacia"}/${payload.quotation.id}`);
        router.refresh();
        return;
      }
      router.refresh();
