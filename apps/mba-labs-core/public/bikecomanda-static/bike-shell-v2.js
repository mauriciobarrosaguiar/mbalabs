(function () {
  ...
  const STYLE_ID = "bikecomanda-shell-v2-style";
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      .bike-loading-topbar {
        ...
        border-bottom: 1px solid rgba(15,138,95,.12);
        box-shadow: 0 8px 24px rgba(14,35,28,.08);
        color: #087354;
        font-size: 24px;
        font-weight: 900;
      }
      .bike-loading-topbar span {
        display: inline-grid;
        place-items: center;
        width: 38px;
        height: 38px;
      ...
      .bike-loading-card {
        ...
        padding: 18px;
        background: #fff;
        box-shadow: 0 12px 30px rgba(14,35,28,.08);
      }
      .bike-loading-card strong { display: block; font-size: 22px; margin-bottom: 6px; }
      .bike-loading-card p { margin: 0; color: #65736e; line-height: 1.45; }
      .bike-menu-toggle, .bike-drawer, .bike-drawer-backdrop { display: none; }
      ...
      @media (max-width: 900px) {
        ...
        .app-shell.bike-fixed-shell .desktop-menu, .app-shell.bike-fixed-shell .mobile-nav { display: none !important; }
        .bike-menu-toggle {
        ...
        .bike-menu-toggle span, .bike-menu-toggle span::before, .bike-menu-toggle span::after {
          display: block;
          width: 21px;
          height: 2px;
          border-radius: 999px;
        ...
        .bike-menu-toggle span::before, .bike-menu-toggle span::after { content: ""; position: absolute; left: 0; }
        ...
        .bike-drawer .user-card { display: block !important; margin: 0 0 12px; border-radius: 16px; }
        .bike-drawer .nav { display: grid !important; grid-template-columns: 1fr !important; gap: 8px !important; width: 100% !important; margin: 0 !important; padding: 0 !important; overflow: visible !important; }
        ...
        .bike-drawer .nav-button.is-active { background: var(--primary-soft) !important; border-color: rgba(15,138,95,.28) !important; color: var(--primary-strong) !important; }
        .bike-drawer .btn.full { width: 100% !important; min-height: 46px !important; margin-top: 12px !important; border-radius: 14px !important; }
      }
    `;
  ...
  function loadingScreen() {
    ...
    root.innerHTML = `
      <main class="bike-loading-page">
        <div class="bike-loading-topbar"><span>BC</span> BikeComanda</div>
        <section class="bike-loading-card">
          <strong>Carregando</strong>
          <p>Sincronizando os dados da empresa.</p>
        </section>
      </main>
    `;
  }

  function openMenu() { document.body.classList.add(OPEN_CLASS); syncToggle(); }
  function closeMenu() { document.body.classList.remove(OPEN_CLASS); syncToggle(); }
  function toggleMenu() { document.body.classList.contains(OPEN_CLASS) ? closeMenu() : openMenu(); }
  ...
  document.addEventListener("click", function (event) {
    ...
    if (target.closest('.bike-drawer [data-action="nav"]')) closeMenu();
    if (target.closest('.bike-drawer [data-action="logout"]')) closeMenu();
  }, true);
